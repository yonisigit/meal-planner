...
import { fetchGuestDishRanks, saveGuestDishRank } from "../api/mealsApi";
import type { GuestDishRank } from "../types";
...
export function GuestDishesModal({ guestId, guestName, mealId, onRemoveGuest, onClose }: GuestDishesModalProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = await fetchGuestDishRanks(guestId);
        if (!mounted) return;
        setDishes(data);
        setError(null);
      } catch (error: unknown) {
  ...
  const refreshGuestDishes = useCallback(async () => {
    const data = await fetchGuestDishRanks(guestId);
    setDishes(data);
  }, [guestId]);
  ...
  async function saveRank(dishId: string, rank: number | null) {
    try {
      ...
      await saveGuestDishRank(guestId, dishId, rank);
      toast.success("Rank saved");
      await refreshGuestDishes();
    } catch (error: unknown) {
